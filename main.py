...
obj = ObjLoader("objects/wolf.obj")
...
# camera properties
eye = (0,0,dia)       # eye is at 2 unit distance from the origin
...
gui = SimpleGUI("Perspective")
fov_slider = gui.add_slider("Field of View", min_value= 0, max_value= 130, initial_value=70)
rotateY_slider = gui.add_slider("Camera Y Angle", min_value=-180, max_value=180, initial_value=0.0)
rotateX_slider = gui.add_slider("Camera X angle", min_value=-90, max_value=90, initial_value=0.0)
...
while draw:
    ...
    rotY_mat = pyrr.matrix44.create_from_y_rotation(np.deg2rad(rotateY_slider.get_value()))
    rotX_mat = pyrr.matrix44.create_from_x_rotation(np.deg2rad(rotateX_slider.get_value()))
    rot_max = pyrr.matrix44.multiply(rotY_mat,rotX_mat)
    rotated_eye = pyrr.matrix44.apply_to_vector(rot_max, transformed_eye)
    view_mat = pyrr.matrix44.create_look_at(rotated_eye, target, up)
    projection_matrix = pyrr.matrix44.create_perspective_projection_matrix(fov_slider.get_value(),aspect,near,far)
    
    glUniformMatrix4fv(view_mat_loc, 1, GL_FALSE, view_mat)
    glUniformMatrix4fv(projection_mat_loc, 1, GL_FALSE, projection_matrix)
    # Draw the triangle
